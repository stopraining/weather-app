...
export default {
                ...
                tooltip: {
                    callbacks: {
                        label: function(tooltipItems) {
                            return tooltipItems.dataset.label+':'+tooltipItems.raw +'°C'
                        },
    ...
    watch:{
        async showChart(newValue){
            if(newValue){
                ...
                let getWeek = await axios.get(`https://opendata.cwb.gov.tw/api/v1/rest/datastore/${weekCountryCode}?Authorization=${this.apiKey}`)
    ...
    methods:{
        weekArray(dataArray){   //low and high temp array data clean
            let resultArray = []
            dataArray.map((x)=>{
        ...
        weekDateArray(dataArray){  //x-axis clean
            let resultArray = []
            dataArray.map((x)=>{
